...
const getDetailedWishlist = AsyncWrapper(async (req, res) => {
...
const deleteFromWishlist = AsyncWrapper(async (req, res) => {
...
const transferIntoCart = AsyncWrapper(async (req, res) => {
    const userId = jwt.verify(req.cookies.auth, process.env.JWT_SECRET).id
    const user = await User.findById(userId)
